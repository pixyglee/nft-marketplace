...
const Searchbar = ({activeSelect, setActiveSelect, handleSearch, clearSearch}) => {
    ...
    return (
        ...
        <div onClick={() =>setToggle((prev) => !prev)} className='relative flexBetween ml-4 sm:ml-0 sm:mt-2 min-w-190 cursor-pointer  dark:bg-nft-black-2 bg-white border dark:border-nft-black-2 border-nft-gray-2 px-4 rounded-md py-3'>
        ...
        />
        {toggle &&(
            <div className="absolute top-full left-0 right-0 mt-3 z-10 dark:bg-nft-black-2 bg-white border dark:border-nft-black-2 border-nft-gray-2 py-3 px-4 rounded-md">
            ...
            {['Recently added','Price (low to high)','Price (high to low)'].map((item)=>(
                ...
                onClick={() =>setActiveSelect(item)} key={item}>{item}</p>
            ))}
            </div>
        )}
        </div>
